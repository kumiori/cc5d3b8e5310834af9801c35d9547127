...
from ui import apply_theme, display_centered_prompt, microcopy, set_page, sidebar_debug_state

MINT_RESULT_KEY = "splash_mint_result"
OPEN_MINT_KEY = "focus_mint_token"


def _build_mint_result(access_key: str, payload: Dict[str, Any], mint_name: str) -> Dict[str, Any]:
...
def _render_access_cta() -> None:
    ...
    with col_login:
        ...
        if st.button("🔑 Go to Login", type="secondary", use_container_width=True, key="splash-go-login"):
...
def _render_mint_result() -> None:
    ...
    if st.button("Go to Login with emoji-4", type="secondary", use_container_width=True):
        st.session_state["login_access_key_prefill"] = str(mint_result.get("emoji4", "")).strip()
        st.session_state["login_access_key_prefill_notice"] = "Going to login. Reloading with your 4-emoji key prefilled."
